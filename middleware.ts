...
export function middleware(request: NextRequest) {
  ...
  let rateLimitConfig: { maxRequests: number; windowMs: number } = RATE_LIMITS.DEFAULT;

  // Login endpoint - strict rate limiting
  if (pathname === '/api/auth/login') {
    rateLimitConfig = RATE_LIMITS.LOGIN as { maxRequests: number; windowMs: number };
  }
  // Admin APIs - higher limit
  else if (pathname.startsWith('/api/admin/')) {
    rateLimitConfig = RATE_LIMITS.ADMIN_API as { maxRequests: number; windowMs: number };
  }
  // Public APIs - moderate limit
  else if (pathname.startsWith('/api/public/') || pathname.startsWith('/api/')) {
    rateLimitConfig = RATE_LIMITS.PUBLIC_API as { maxRequests: number; windowMs: number };
  }
